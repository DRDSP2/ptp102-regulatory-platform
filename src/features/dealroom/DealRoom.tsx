...
export default function DealRoom() {
    ...
    return (
      <div className="min-h-screen bg-[var(--page-bg)] text-ink-900">
        <div className="max-w-3xl mx-auto p-6">
          ...
          <p className="text-sm text-ink-500 mb-4">You must execute the NDA before accessing deal room materials.</p>
          <div className="border border-ink-200 rounded-xl bg-white/60 p-5 text-sm text-ink-600 space-y-2 mb-6">
  ...
  return (
    <div className="min-h-screen bg-[var(--page-bg)] text-ink-900 flex">
      <aside className="w-64 bg-white border-r border-ink-200 flex flex-col">
        <div className="p-5 border-b border-ink-200">
          ...
          <p className="text-xs text-ink-500 mt-1">PTP-102 Deal Room</p>
        </div>
          ...
          {[
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-3 py-2 rounded-xl text-sm transition ${
                  page === item.key ? 'bg-sky-700 text-white' : 'text-ink-600 hover:bg-ink-100'
                }`}
              >
        ...
        <div className="p-4 border-t border-ink-200 space-y-2">
          ...
          <p className="text-xs text-ink-500 truncate">{profile.company}</p>
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm text-ink-500 hover:text-white transition"
          >
      ...
      <main className="flex-1 p-6 overflow-auto">
        {page === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="border border-ink-200 rounded-xl bg-white/60 p-5">
                <div className="flex items-center gap-4 mb-2">
                  <FolderOpen className="w-5 h-5 text-sky-400" />
                  <h3 className="font-semibold">Data Room</h3>
                </div>
                <p className="text-sm text-ink-500 mb-3">Access pitch deck, cap table, CMC dossier and term sheet templates.</p>
              ...
              <div className="border border-ink-200 rounded-xl bg-white/60 p-5">
                ...
                <p className="text-sm text-ink-500 mb-3">Draft and submit regional licence proposals.</p>
              ...
              <div className="border border-ink-200 rounded-xl bg-white/60 p-5">
                ...
                <p className="text-sm text-ink-500 mb-3">Browse global territories available for PTP-102 licensing.</p>
            ...
            <div className="border border-ink-200 rounded-xl bg-white/60 p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between border-b border-ink-200 pb-1"><span>Target Conditional Approval</span><span className="font-medium">3Q28</span></div>
                <div className="flex justify-between border-b border-ink-200 pb-1"><span>Non-CMC Base Cost</span><span className="font-medium">~$519k</span></div>
                <div className="flex justify-between border-b border-ink-200 pb-1"><span>Indicative Royalty</span><span className="font-medium">5%</span></div>
        ...
        {page === 'trials' && (
          <div className="space-y-6">
            ...
            {lockedTrial ? (
              <div className="border border-ink-200 rounded-xl bg-white/60 p-8 text-center">
                <Lock className="w-10 h-10 text-amber-400 mx-auto mb-3" />
                <h2 className="text-xl font-bold">Due Diligence Tier Required</h2>
                <p className="text-ink-500 mt-2">Live trial feeds are available from the Due Diligence tier onwards.</p>
              </div>
            ) : (
                  ...
                  <div className="space-y-2">
                    {chartData.map((point) => (
                      <div key={point.hour} className="flex items-center gap-4 text-sm">
                        <span className="w-16 text-ink-500">{point.hour}h</span>
                        ...
                        <span className="w-32 text-ink-600">{point.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                      ...
                      <tbody className="divide-y divide-ink-200">
                        {TRIAL_EVENTS.map((e) => (
                          <tr key={e.id}>
                            ...
                            <td className="py-2 text-ink-500">{new Date(e.timestamp).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
function PitchView() {
  ...
  return (
    <div className="space-y-4">
      <div className="border border-ink-200 rounded-xl bg-white/60 p-8 min-h-[360px] flex flex-col justify-between">
        <div>
          <h2 className="text-3xl font-bold mb-6">{slide.title}</h2>
          <ul className="space-y-4">
            {slide.bullets.map((b, i) => (
              <li key={i} className="flex items-start gap-4 text-lg text-ink-800">
        ...
        <div className="flex items-center justify-between mt-8 pt-6 border-t border-ink-200">
          <button onClick={() => setIdx((i) => Math.max(0, i - 1))} disabled={idx === 0} className="flex items-center gap-1 px-4 py-2 rounded bg-ink-100 hover:bg-ink-200 disabled:opacity-40"><ChevronLeft className="w-4 h-4" /> Previous</button>
          <span className="text-sm text-ink-500">{idx + 1} / {PITCH_SLIDES.length}</span>
...
function DataRoomView({ profile }: { profile: UserProfile }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 border border-ink-200 rounded-xl bg-white/60 px-3 py-2">
        ...
        <input
          ...
          className="bg-transparent flex-1 text-sm focus:outline-none text-ink-900 placeholder:text-ink-400"
        />
      </div>
      <div className="grid gap-3">
        {docs.map((doc) => {
          ...
          return (
            <div key={doc.id} className={`flex items-center justify-between p-5 rounded-xl border ${access ? 'border-ink-200 bg-white/60' : 'border-ink-200/50 bg-white/50'}`}>
              <div className="flex items-start gap-3">
                <FileText className={`w-5 h-5 mt-0.5 ${access ? 'text-sky-400' : 'text-ink-400'}`} />
                <div>
                  <h3 className={`font-medium ${access ? '' : 'text-ink-400'}`}>{doc.title}</h3>
                  <p className="text-xs text-ink-500">{doc.category} • {doc.filename} • {doc.version}</p>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm transition ${access ? 'bg-sky-700 hover:bg-sky-600 text-white' : 'bg-ink-100 text-ink-400 cursor-not-allowed'}`}
              >
...
function TermSheetView({
  ...
  if (!active) return <p className="text-ink-500">No term sheets yet.</p>;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-500 text-sm">Draft, sign and submit PTP-102 regional licences for legal review.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="border border-ink-200 rounded-xl bg-white/60 p-5 space-y-2">
          <h3 className="font-semibold text-sm text-ink-600">Active Term Sheets</h3>
          {terms.map((s) => (
            <button
              key={s.id}
              onClick={() => setActiveId(s.id)}
              className={`w-full text-left px-3 py-2 rounded text-sm transition ${s.id === activeId ? 'bg-sky-900 text-white' : 'hover:bg-ink-100 text-ink-600'}`}
            >
              {regionLabel(s.region)} — {s.status}
            </button>
          ))}
        </div>
        <div className="lg:col-span-2 border border-ink-200 rounded-xl bg-white/60 p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-ink-600 mb-1">Licensee</label>
              <input
                type="text"
                value={active.prospect_company}
                onChange={(e) => onChange(active.id, { prospect_company: e.target.value })}
                className="w-full rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink-600 mb-1">Region</label>
              <select
                value={active.region}
            ...
            <div>
              <label className="block text-sm font-medium text-ink-600 mb-1">Upfront Fee (USD)</label>
              <input type="number" value={active.upfront_fee} onChange={(e) => onChange(active.id, { upfront_fee: Number(e.target.value) })} className="w-full rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink-600 mb-1">Royalty Rate (%)</label>
              <input type="number" step={0.1} value={active.royalty_rate} onChange={(e) => onChange(active.id, { royalty_rate: Number(e.target.value) })} className="w-full rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink-600 mb-1">Exclusivity (months)</label>
              <input type="number" value={active.exclusivity_months} onChange={(e) => onChange(active.id, { exclusivity_months: Number(e.target.value) })} className="w-full rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm" />
            </div>
          </div>

          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-ink-600">Milestone Payments</label>
              <button onClick={onAddMilestone} className="flex items-center gap-1 text-xs bg-ink-100 hover:bg-ink-200 px-2 py-1 rounded"><Plus className="w-3 h-3" /> Add</button>
            </div>
            <div className="space-y-2">
              {active.milestones.map((m, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm"
                  ...
                  <input
                    ...
                    className="w-32 rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm"
                    placeholder="USD"
                  />
                  <button onClick={() => onRemoveMilestone(idx)} className="p-2 text-ink-500 hover:text-red-400"><X className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <button
              ...
              className="flex items-center gap-2 bg-ink-100 hover:bg-ink-200 text-white px-4 py-2 rounded text-sm"
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded text-sm ${active.status === 'draft' ? 'bg-sky-600 hover:bg-sky-500 text-white' : 'bg-ink-100 text-ink-400 cursor-not-allowed'}`}
            >
              <Send className="w-4 h-4" /> Submit for Legal Review
            </button>
          </div>
          <p className="text-xs text-ink-400">No payment is processed here. Terms are submitted to Byrock legal for review and execution.</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {REGIONS.map((r) => (
          <button
            ...
            className={`text-left p-5 rounded-xl border transition ${selected === r.region ? 'border-amber-400 bg-ink-100' : 'border-ink-200 bg-white/60 hover:bg-ink-100'}`}
          >
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold">{r.label}</h3>
              {r.status === 'licensed' ? <Lock className="w-4 h-4 text-ink-500" /> : <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
            </div>
            <p className="text-xs text-ink-500 mb-3">Base licence fee indicative</p>
      ...
      {selected && (
        <div className="border border-ink-200 rounded-xl bg-white/60 p-5">
          ...
          <p className="text-sm text-ink-500 mb-4">This will create a new term sheet draft with the region’s indicative upfront fee and default milestones.</p>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="border border-ink-200 rounded-xl bg-white/60 p-5">
        ...
        <div className="space-y-2">
          {capTableData.map((row, idx) => (
            <div key={row.name} className="flex items-center justify-between p-5 rounded border border-ink-200 bg-[var(--page-bg)]">
              ...
              <div className="text-right text-sm">
                ...
                <p className="text-ink-500 text-xs">{row.shares.toLocaleString()} shares</p>
              </div>
            </div>
          ))}
        </div>
