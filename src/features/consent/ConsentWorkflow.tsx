...
export default function ConsentWorkflow() {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-ink-500">Generate 21 CFR Part 11 compliant electronic consent documents with e-signatures.</p>
          ...
          {selectedPatient ? (
            <div className="p-4 space-y-4">
              <div className="border-b border-ink-200 pb-3">
                ...
                <p className="text-sm text-ink-500">Owner: {selectedPatient.owner_name} | Status: {selectedPatient.status}</p>
              </div>

              <div className="space-y-4">
                <label className="block">
                  <span className="text-sm text-ink-600">Owner Name (for signature)</span>
                  <input
                    type="text"
                    value={ownerName}
                    onChange={e => setOwnerName(e.target.value)}
                    className="mt-1 w-full rounded border border-ink-200 bg-[var(--page-bg)] px-2 py-1.5 text-sm"
                    placeholder="Full legal name of owner"
                  />
                </label>

                <label className="block">
                  <span className="text-sm text-ink-600">Electronic Signature (Owner)</span>
                  <div className="relative mt-1">
                    <canvas
                      ref={signaturePadRef}
                      width={600}
                      height={200}
                      className="border border-ink-300 bg-white rounded cursor-crosshair"
                    ...
                    {signatureImg && (
                      <div className="absolute top-2 right-2 flex gap-1">
                        ...
                        <button type="button" onClick={handleDownload} className="p-1 rounded bg-white/50 text-ink-600 hover:bg-white/60 text-xs">Download PNG</button>
                      </div>
                    )}
                  </div>
                  <p className="text-xs text-ink-400 mt-1">Owner draws signature with mouse/touch. 21 CFR Part 11 compliant e-signature.</p>
                </label>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-ink-200">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded bg-ink-100 text-ink-900 font-medium disabled:opacity-50"
                  >
                    <FileText className="h-4 w-4" />
                    {generating ? 'Generating PDF...' : 'Generate Consent PDF'}
                  </button>
                  {_consent?.document_url && (
                    <a href={_consent.document_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded border border-ink-200 text-sm hover:bg-ink-100">
                ...
                <div className="p-4 bg-white/60 rounded border border-ink-200 text-sm text-ink-500">
